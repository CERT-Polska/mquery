...
class Agent:
    ...
    def db_id(self, session: Session) -> int:
        self.__db_id = self.db.get_active_agents(session)[self.group_id].id
        assert self.__db_id is not None
        return cast(int, self.__db_id)
    ...
    def update_metadata(
        ...
        self.db.add_match(session, job, match)

    def execute_yara(
        self, session: Session, job: Job, files: List[str]
    ) -> None:
        rule = yara.compile(source=job.raw_yara)
        num_matches = 0
        num_errors = 0
        num_files = len(files)
        self.db.job_start_work(session, job.id, num_files)
        ...
        new_processed = self.db.job_update_work(
            session, job.id, num_files, num_matches, num_errors
        )
    ...
    def init_search(self, session: Session, job: Job, tasks: int) -> None:
        self.db.init_jobagent(session, job, self.db_id(session), tasks)

    def add_tasks_in_progress(
        self, session: Session, job: Job, tasks: int
    ) -> None:
        """See documentation of db.add_tasks_in_progress."""
        self.db.add_tasks_in_progress(session, job, self.db_id(session), tasks)
...
def job_context(job_id: JobId):
    ...
    with agent.db.session() as session:
        try:
            ...
            agent.db.fail_job(session, job_id, str(e))
            session.commit()
            raise
...
def start_search(job_id: JobId) -> None:
    ...
    units of work.
    """
    with job_context(job_id) as (agent, session):
        job = agent.db.get_job(session, job_id)
        if job.status == "cancelled":
            logging.info("Job was cancelled, returning...")
            return

        datasets = agent.get_datasets()
        agent.db.init_job_datasets(session, job_id, len(datasets))
        ...
        agent.init_search(session, job, len(datasets))
...
def query_ursadb(job_id: JobId, dataset_id: str, ursadb_query: str) -> None:
    ...
    with job_context(job_id) as (agent, session):
        ...
        total_files = agent.db.update_job_files(session, job_id, file_count)
        ...
        agent.add_tasks_in_progress(session, job, len(batches) - 1)
        ...
        agent.db.dataset_query_done(session, job_id)
...
def run_yara_batch(job_id: JobId, iterator: str, batch_size: int) -> None:
    ...
    with job_context(job_id) as (agent, session):
        ...
        agent.execute_yara(session, job, pop_result.files)
        agent.add_tasks_in_progress(session, job, -1)
