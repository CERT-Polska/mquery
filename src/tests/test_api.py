...
def add_files_to_index(check_operational):
    ...
    for i in range(1, num_files):
        words_list.append(''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(word_length)))
    ...
    for i in range(1, 6):
        files_to_detect[i + 4] = files_to_detect[i - 1] + files_to_detect[i + 4]
...
def request_query(log, i):
    res = requests.post('http://web:5000/api/query/low', json={'method': 'query', 'raw_yara': i})
